...
print("Image width in meter {} at {} from camera".format("%.0f" % frame_width_ft,"%.0f" % DISTANCE))
    ...
    if motion_found:
        if state == WAITING:
            ...
            if state == TRACKING:       
                ...
                kmh = get_speed(abs_chg,ftperpixel,secs)
                print("--> chg={}  secs={}  kmh={} this_x={} w={} ".format(abs_chg,secs,"%.0f" % kmh,x,w))
                ...
                if ((x <= 2) and (direction == RIGHT_TO_LEFT)) \
                    ...
                    size, base = cv2.getTextSize( "%.0f kmh" % last_kmh, cv2.FONT_HERSHEY_SIMPLEX, 2, 3)
                    # then center it horizontally on the image
                    cntr_x = int((IMAGEWIDTH - size[0]) / 2) 
                    cv2.putText(image, "%.0f kmh" % last_kmh,
